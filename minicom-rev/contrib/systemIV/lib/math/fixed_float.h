...
class Fixed
{
	...
	public:
		...
		{
			double dif = _rhs.m_value - m_value;
			return ( dif >= SMALLEST_DOUBLE );
		...
		inline bool operator == (const Fixed& _rhs) const
		{
			double dif = m_value - _rhs.m_value;
			return ( -SMALLEST_DOUBLE < dif && dif < SMALLEST_DOUBLE );
			//return m_value == _rhs.m_value;
		}
