...
bool MovingObject::IsMovingObject()
{	
		return true;
}
...
bool MovingObject::IsValidPosition ( Fixed longitude, Fixed latitude )
{
    ...
    if(!g_app->GetMapRenderer()->IsValidTerritory( -1, longitude, latitude, false ) ) validWaypoint = true;
...
WorldObject * MovingObject::GetClosestLandingPad()
{
    ...
    for( int i = 0; i < world->m_objects.Size(); ++i )
    {
        if( world->m_objects.ValidIndex(i) )
        {
            ...
            if( obj->m_teamId == m_teamId && CanLandOn(obj->m_type))
            {
...
Fixed MovingObject::GetSpeed()
{
	return m_settings->m_speed;
}
