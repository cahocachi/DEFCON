...
void AirBase::Render( RenderInfo & renderInfo )
{
    ...
    {   
        ...
        if( bmpImage )
        {
            float x = renderInfo.m_position.x - GetSize().DoubleValue() * 0.5f;
            float y = renderInfo.m_position.y - GetSize().DoubleValue() * 0.3f;    
...
int AirBase::GetNukeCount()
{
	if( m_nukeSupply > 0 )
    {
		return (m_states[1]->m_numTimesPermitted);
    }
...
bool AirBase::LaunchBomber( WorldObjectReference const & targetObjectId, Fixed longitude, Fixed latitude )
{
...
bool AirBase::LaunchFighter( WorldObjectReference const & _targetObjectId, Fixed longitude, Fixed latitude )
{
