...
Fixed Fleet::GetSpeed()
{
	Fixed slowest = Fixed::MAX;
	MovingObject *mobj;
    ...
    for( int i = 0; i < m_fleetMembers.Size(); ++i )
    {
		...
		if(obj && obj->IsMovingObject()) //safety check
			mobj = (MovingObject *) obj;
		else
			continue;

		Fixed objspeed = mobj->GetSpeed();
		if(objspeed <slowest)
			slowest = objspeed;
