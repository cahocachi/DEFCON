...
void MapRenderer::RenderCityObjectDetails( WorldObject *wobj, float *boxX, float *boxY, float *boxW, float *boxH, float *boxSep )
{
    ...
    *boxY -= *boxH;
    ...
    char *objName = wobj->GetName( wobj->m_type );
...
void MapRenderer::RenderMouse()
{
    ...
    if( selection )
    {
        ...
        else if( validMovementTarget > 0 )
        {
            ...
            // check for landing
			if( highlight && selection->CanLandOn(highlight->m_type))
            {   
    ...
    if( team )
    {
        for( int i = 0; i < g_app->GetWorld()->m_objects.Size(); ++i )
        {
            if( g_app->GetWorld()->m_objects.ValidIndex(i) )
            {
                ...
                {
					int nukeCount = obj->GetNukeCount();
...
void MapRenderer::RenderObjects()
{
    ...
    for( int i = 0; i < world->m_objects.Size(); ++i )
    {
        ...
        {            
            ...
            START_PROFILE( wobj->GetName(wobj->m_type) );
...
#endif
            END_PROFILE( wobj->GetName(wobj->m_type) );
        }
    }
        ...
        if( obj )
        {
            ...
            else if( numTimesRemaining == -1 || numTimesRemaining > 0 )
            {
                ...
                if( !underMouse )
                {
                    ...
                    {
                        canAction = obj->CanLaunchAnywhere();
                    }
